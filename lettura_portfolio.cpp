...
int main()
{
    ...
    while (std::getline(file, line))
    {
        ...
        size_t pos_percentuale = valoreTemporale.find(' ');  // Trova la posizione dello spazio prima della parentesi
        if (pos_percentuale != std::string::npos) {
            ...
            valore_temporale = std::stof(estratto); // Converti la stringa in float
        }

        // Verifica se l'opzione è venduta (posizione < 0) e se il valore temporale è minore di 1
        if (int_Posizione < 0 && valore_temporale < 1.0)
        {
            std::cout << "ALLARME: Opzione venduta con valore temporale inferiore a 1$!" << std::endl;
            std::cout << "Simbolo: " << simbolo_scadenza << ", Valore temporale: " << valore_temporale << "$" << std::endl;
        }
